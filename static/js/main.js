...
(function(){
  ...
  // ── 代码高亮双主题（giallo-light/dark.css）跟随 data-theme ──
  function syncGialloTheme(){
    try{
      var isDark = getIsDark();
      var l = document.getElementById('giallo-light');
      var d = document.getElementById('giallo-dark');
      if(l) l.disabled = isDark;
      if(d) d.disabled = !isDark;
    }catch(e){}
  }
  ...
  function initCodeBlocks(){
    ...
    const pres=document.querySelectorAll('#articleContent pre, .article__content pre, .tabs__panel pre, .col-card pre');
    pres.forEach(pre=>{
      ...
      // ── Highlight lines (hl_lines → z-hl / 双主题 z-l-hl·z-d-hl) — sync gutter with code ──
      try{
        const hlNodes = code.querySelectorAll('.giallo-l.z-hl, .giallo-l.z-l-hl, .giallo-l.z-d-hl, .giallo-l.hll, .z-hl, .z-l-hl, .z-d-hl');
        if(hlNodes.length && gutter){
          ...
          if(codeLines.length === gutterLines.length){
            codeLines.forEach((ln, idx)=>{
              if(ln.classList.contains('z-hl') || ln.classList.contains('z-l-hl') || ln.classList.contains('z-d-hl') || ln.classList.contains('hll')){
        ...
        if(hlAttr && gutter){
          ...
          const gutterLines=gutter.querySelectorAll('.gutter-line');
          const codeLines=code.querySelectorAll('.giallo-l');
          // highlight code lines（双主题需同时加 z-l/z-d，启用中的样式表才会生效）
          if(codeLines.length){
            codeLines.forEach((ln, idx)=>{
              if(hlSet.has(idx+1)) ln.classList.add('z-hl', 'z-l-hl', 'z-d-hl');
            });
          } else {
  ...
  // ── Tabs ── 导航由 JS 按各面板 data-title 生成；只取直接子级，嵌套 tabs 各自独立
  function initTabs(){
    document.querySelectorAll('[data-component="tabs"]').forEach(root=>{
      if(root.dataset.tabsInit==='1') return;
      const nav=root.querySelector(':scope > .tabs__nav');
      const panelsEl=root.querySelector(':scope > .tabs__panels');
      if(!nav||!panelsEl) return;
      const panels=Array.from(panelsEl.querySelectorAll(':scope > .tabs__panel'));
      ...
      function activateTab(idx){
        ...
        if(btns[idx]){btns[idx].classList.add('is-active');btns[idx].setAttribute('aria-selected','true');}
        const ap=panels[idx];
        if(ap){
          ap.classList.add('is-active');
          // Fix code blocks inside tabs: force reflow to ensure gutter aligns after hidden->visible
          ap.querySelectorAll('.code-wrap').forEach(function(w){
            void w.offsetHeight;
            var body=w.querySelector('.code-wrap__body');
            if(body){body.style.overflow='auto';void body.offsetWidth;}
          });
          // also handle yet-unwrapped pre inside panel
          initCodeBlocks();
        }
      }
      nav.innerHTML='';
      panels.forEach((p, idx)=>{
        const label=((p.getAttribute('data-title')||'Tab').trim()||'Tab').toLowerCase();
